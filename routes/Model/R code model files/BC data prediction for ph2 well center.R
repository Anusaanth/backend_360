...
# Final Output 
Pred_result<-cbind(testdata$UWI,pfw_prob[,2])
colnames(Pred_result)=c("UWI","Well.Center.Pass.Probability")


pfw_prob <- as.data.frame(pfw_prob)
Pred_result<-cbind(testdata$UWI,pfw_prob)
colnames(Pred_result)=c("UWI","Phase2.Well.Center.Results")
